...
- Every 6 seconds he plants three warden-style circles around you (they do not follow). Every 12 seconds he calls a ring of 8 bats, which count toward the enemy cap. He also dashes: a red line points at you for 1.0s, then he travels about 400px along it and hits for 25. Below 2500 HP the marks come every 4 seconds, the ring is 12 bats, and a dash follows each bat call.
- At 9:00, `BossUI` (`js/ui/boss.js`) warns **THE VAMPIRE LORD** for 1.6s, then he lands: a flash, a screen shake (`FX.boss('lordLand')`), and a boss HP bar across the top until he dies. The dash telegraph is the same module's blood line. Killing him ends the run on **Lord slain**. Reaching 10:00 while he is still up is still **Dawn breaks**. `begin()` calls `BossUI.reset()` so a death at 9:30 and a restart do not keep the bar before the next 9:00.
