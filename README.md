...
Nightfall is a Vampire Survivors–inspired survival roguelite that runs in the browser. v0.6 is a playable night: move with the keyboard or a touch stick, auto-attack with a stake, an unlockable censer, pyre, and ash cross, pull gems in with a magnet boon, level up, and survive a spawn curve that starts sparse, thickens, and brings a warden around the second minute and again later. Last until dawn at ten minutes. The hunter and the ground are PixelLab pixel art. Weapons, impacts, and gems are CC0 sprites from OpenGameArt, listed in [assets/CREDITS.md](assets/CREDITS.md).
...
Smoke coverage lives under `qa/`. It drives the page in Chromium and asserts through `window.__game` / `weaponSummary()`. The document title pin matches v0.6. How to run it is in [qa/README.md](qa/README.md).
...
- `assets/` — PixelLab hunters (`assets/characters/hunter/`, `assets/characters/stakeman/`), unused backup sheet (`player_sheet.png`), fallback floor (`tileset.png`), enemy sheets (`assets/enemies/`), graveyard floor (`assets/tiles/`), and decorations (`assets/decor/`)
...
- **Pixel-art hunter.** The player is drawn from the sheet on their character row, generated with [PixelLab](https://pixellab.ai). The Hunter is `assets/characters/hunter/hunter_sheet.png` and the Stakeman is `assets/characters/stakeman/stakeman_sheet.png`. Each sheet is 7 columns by 8 rows of 64×64. Rows are south, south-east, east, north-east, north, north-west, west, south-west. Column 0 is idle. Columns 1–6 are a walk cycle at about 10 frames per second, played only while the movement axis is held. Facing is 8-way from that vector and stays on the last direction when you stop. Scaling is nearest-neighbor (`imageSmoothingEnabled = false`). `bodyBox` puts the feet on the entity and the painted body is about 48px tall. The hitbox radius stays 14. A soft shadow sits under the feet. `player.facing` is the row name (`east`, `west`, …). If the sheet fails to load, the previous shape-drawn hunter is used. `assets/player_sheet.png` stays in the repo as an unused backup.
...
- Art for each hunter is the `sheet` on their row in `js/characters.js`. The Hunter loads `assets/characters/hunter/hunter_sheet.png` (64×64, `bodyBox` 18,7,28,53). The Stakeman loads `assets/characters/stakeman/stakeman_sheet.png`. Both use column 0 idle, columns 1–6 walk, and eight facings. The select card and the in-game sprite read that same sheet. `assets/player_sheet.png` is not requested.
...
## v0.6 notes for Engineer and QA

Stacks on the v0.5.2 night. Animated foes, the Stakeman's own sheet, and a boss at 9:00.

- Enemy sheets are read from JSON. Frame count is `walkFrames ?? frames`, and a missing count is 1. Walk frames advance at about 8–10 fps, a little faster as move speed rises, with a random phase so a swarm does not step in unison. `bodyBox` only places the feet on the entity and scales the painted body (bat 32, shambler 44, brute 58, hunters 48, lord about 96). Collision radii are unchanged, including the brute at 20 even though its box is 60×60. Bats still weave sideways. The old static-sprite bob is gone.
- The Hunter is the Nightfall Vampire Hunter sheet at `assets/characters/hunter/hunter_sheet` (64×64, seven columns, `bodyBox` 18,7,28,53). The Stakeman stays on `assets/characters/stakeman/stakeman_sheet`. The card preview and the in-game sprite both use that character's sheet. The old chibi sheet `assets/player_sheet.png` remains in the repo and is not loaded.
- The Vampire Lord spawns at 540s (9:00). The warden that used to spawn at 580s does not; earlier wardens (90/100, 220, 340, 460) are unchanged. He has a flat 5000 HP, speed 55, 20 contact damage, knockback immunity, and a collision radius of 34. Walk, cast, and dash each use their own sheet.
- Every 6 seconds he plants three warden-style circles around you (they do not follow). Every 12 seconds he calls a ring of 8 bats. Like a swarm, that ring spawns past the 220 cap, then those bats count toward it. He also dashes: a red line points at you for 1.0s, then he travels about 400px along it and hits for 25. Below 2500 HP the marks come every 4 seconds, the ring is 12 bats, and a dash follows each bat call.
- At 9:00, `BossUI` (`js/ui/boss.js`) warns **THE VAMPIRE LORD** for 1.6s, then he lands: a flash, a screen shake (`FX.boss('lordLand')`), and a boss HP bar across the top until he dies. The dash telegraph is the same module's blood line. Killing him ends the run on **Lord slain**, with a gold banner. Reaching 10:00 while he is still up, or if he never arrived, is **Dawn breaks — the Lord escapes**. Both are wins. `begin()` calls `BossUI.reset()` so a death at 9:30 and a restart do not keep the bar before the next 9:00.
- **F3** adds `Lord hp/max · Phase n` while he is alive. `window.__game.debugJumpToLord()` sets the clock to 539s and consumes the earlier warden slots so the jump does not replay them.
